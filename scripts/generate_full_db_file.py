...
DATA_DF = DATA.drop(['index', 'tg_numc_scope_1', 'tg_numc_scope_2', 'tg_numc_scope_3'], axis=1).merge(SCP.drop('index', axis=1),
                                                                                                              ...
                                                                                                              suffixes=('', '_DROP'))
# DATA_DF = DATA_DF.merge(LAR.drop('index', axis=1), how='left', on=['key_ticker', 'key_year'], suffixes=('', '_DROP'))
DATA_DF = DATA_DF.merge(CMP.drop('index', axis=1), how='left', on=['key_ticker'], suffixes=('', '_DROP'))
