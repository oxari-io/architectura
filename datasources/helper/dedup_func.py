...
def name_and_exchange_priority_based_deduplication(df:pd.DataFrame, exchange_ranking_path:str) -> pd.DataFrame:
    ...
    grouped_df = df_ranked.groupby(['key_year', 'meta_name']).ffill()
    grouped_df[['key_year', 'meta_name']] = df_ranked[['key_year', 'meta_name']].copy()
    grouped_df = grouped_df.groupby(['key_year', 'meta_name']).bfill()
    grouped_df[['key_year', 'meta_name']] = df_ranked[['key_year', 'meta_name']].copy()
...
def categorical_name_and_exchange_priority_based_deduplication(df:pd.DataFrame, exchange_ranking_path:str) -> pd.DataFrame:
    MAIN_NAME_COL = 'meta_name'
    exchange_df = load_exchange_ranking(exchange_ranking_path)
    # merge rankings
    # note, clean exchange names are also available in meta_exchange_display_name after this merge
    df_ranked = pd.merge(df, exchange_df, on='meta_exchange', how='left')
    ...
    # keep the other ticker combinations
    # adjust the aggregation to exclude the first ticker
    alternative_ticker_df = grouped_df.groupby([MAIN_NAME_COL]).apply(lambda x: list(set(list(x['key_ticker'])))).reset_index(name='meta_ticker_list')

    # merge back
    top_priority_df = pd.merge(top_priority_df, alternative_ticker_df, on=[MAIN_NAME_COL], how='left')
